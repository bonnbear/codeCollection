...
## 步驟 7：切換至 MongoDB Atlas 資料庫 (取代 api/db.json)

為了將資料持久化從本地檔案 `api/db.json` 升級到雲端資料庫，您需要使用 MongoDB Atlas。

### 7.1. MongoDB Atlas 帳戶註冊與集群建立

1.  **註冊帳戶並登入**：前往 MongoDB Atlas 網站 ([`https://www.mongodb.com/cloud/atlas`](https://www.mongodb.com/cloud/atlas)) 進行註冊和登入。
2.  **建立集群**：
    *   點擊 **Build a Database**。
    *   選擇 **M0 (Shared)** 免費層級 (Free Tier)。
    *   選擇您偏好的雲服務供應商和地區，並為集群命名。
3.  **設定資料庫存取權限 (Database Access)**：
    *   建立一個新的資料庫使用者 (例如 `vercel-user`)，並設定一個**強密碼**。**請務必記住此使用者名稱和密碼。**
4.  **設定網路存取權限 (Network Access)**：
    *   點擊 **Add an IP Address**。
    *   為了讓 Vercel 能夠連接，建議選擇 **Allow Access from Anywhere** (將 IP 位址設定為 `0.0.0.0/0`)。

### 7.2. 獲取 MongoDB 連接字串

1.  **導航到集群**：在集群概覽頁面，等待集群建立完成。
2.  **點擊連接**：點擊集群名稱旁邊的 **Connect** 按鈕。
3.  **選擇連接方式**：選擇 **Connect your application** (連接您的應用程式)。
4.  **複製連接字串**：
    *   選擇 **Node.js** 驅動程式和最新版本。
    *   複製以下格式的字串：
        ```
        mongodb+srv://<username>:<password>@<cluster-url>/<dbname>?retryWrites=true&w=majority
        ```
5.  **替換憑證**：
    *   **手動將 `<username>` 替換為您在 7.1 步驟 3 中設定的資料庫使用者名稱。**
    *   **手動將 `<password>` 替換為您在 7.1 步驟 3 中設定的密碼。**
    *   **可選：** 將 `<dbname>` 替換為您希望應用程式使用的資料庫名稱 (例如 `my-todo-app`)。

### 7.3. 在 Vercel 中設定 MONGODB_URI

1.  **登入 Vercel 儀表板**：登入您的 Vercel 帳戶並導航到您的專案。
2.  **進入環境變數設定**：導航到 **Settings** (設定) -> **Environment Variables** (環境變數)。
3.  **新增 MONGODB_URI**：
    *   **Name (名稱):** `MONGODB_URI`
    *   **Value (值):** 貼上您在 7.2 步驟 5 中替換了實際憑證的完整連接字串。
    *   **Environments (環境):** 選擇 **Development**、**Preview** 和 **Production**。
4.  **儲存並重新部署**：點擊 **Save**，然後導航回專案的 **Overview** 頁面點擊 **Redeploy**，以使新的環境變數生效。