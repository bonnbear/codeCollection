...
function isTopLevelVariableArrowFunctionStart(lines, block, lineNo) {
   ...
   *   () => {
   *
   * 从当前行往后看几行，只要在语句结束前看到 =>，就认为是箭头函数。
   */
  const maxLookAheadLine = Math.min(block.endLine, lineNo + 6);
