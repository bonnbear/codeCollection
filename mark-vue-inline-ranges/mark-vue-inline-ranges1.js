...
function processVueFile(filePath, rawRanges, options) {
  ...
  for (const [lineNo, mark] of entries) {
    ...
    if (block.type === "template") {
      ...
      newLine = appendCommentWithSpace(originalLine, `//${payload}`);
  ...
  if (!changedCount) {
    console.log(`[完成] ${filePath}，没有新增标记`);
    return;
  }

  if (options.dryRun) {
    console.log(`[预览] ${filePath}，将标记 ${changedCount} 行，未写入`);
    return;
  }
...
function appendTemplateMarkSafely(lines, blocks, lineNo, line, payload, options) {
   ...
   * 避免：
   *
   * <div v-if="ok"></div> <!--mark-->
   * <div v-else></div>
   *
   * 这类场景可能影响 Vue 对 v-else 邻接关系的判断。
   */
